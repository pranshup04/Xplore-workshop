...
def insert_item(name: str, price: float) -> int:
   ...
   conn = get_conn()
   cur = conn.cursor()
   cur.execute("INSERT INTO items (name, price) VALUES (?, ?)", (name, price)) #removed int casting to allow point values for price
   conn.commit()
   rowid = cur.lastrowid
   conn.close()
   return rowid
...
def update_item(item_id: int, name: str = None, price: float = None) -> bool:
   ...
   sql = f"UPDATE items SET {', '.join(updates)} WHERE id = ?"  # updates only one id
...
def delete_item(item_id: int) -> bool:
   ...
   conn.commit()
   conn.close()
   return affected > 0  #return true only if a row was removed
